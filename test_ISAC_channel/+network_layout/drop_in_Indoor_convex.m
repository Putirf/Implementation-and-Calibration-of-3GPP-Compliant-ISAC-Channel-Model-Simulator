function pos = drop_in_Indoor_convex(trp_pos_list, ue_pos_list, xy_range, min_d_trp, min_d_ue)
...
%   pos          : [1 x 2] target position (x,y)
    ...
    % --- Build convex hull polygon of TRPs ---
    x = trp_pos_list(:,1); y = trp_pos_list(:,2);
    k = convhull(x, y);           % closed polygon indices (last repeats first)
    hx = x(k); hy = y(k);         % hull polygon vertices (closed)
    ...
    while true
        % 1) Uniform in rectangle
        Px = xmin + (xmax - xmin) * rand;
        Py = ymin + (ymax - ymin) * rand;
        P  = [Px, Py];

        % 2) Reject if outside TRP convex hull
        inside = inpolygon(Px, Py, hx, hy);  % true if inside/on boundary
        if ~inside
            continue;
        end

        % 3) Optional min distance to TRPs
        if min_d_trp > 0
        ...
        % 4) Optional min distance to UEs
