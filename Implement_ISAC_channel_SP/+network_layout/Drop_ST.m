...
if isempty(St_pos)
    for st = 1:scenario.ST_per_cell
        ...
        elseif strcmp(scenario.name, 'UrbanGrid')
            % TEMP UrbanGrid calibration: enforce TRP/UE minimum distance rejection for vehicle targets.
            % Original state: [ST_pos_2D, side, ~] = network_layout.drop_Vehicle_ST(scenario);
            [ST_pos_2D, side, ~] = localDropUrbanGridVehicleST( ...
                BS_list, UE_pos_list, scenario, BS_ST_min_d, UE_ST_min_d);
            St.phi_v = (side - 1) * 90;
...
function [ST_pos_2D, side, lane_id] = localDropUrbanGridVehicleST( ...
    BS_list, UE_pos_list, scenario, BS_ST_min_d, UE_ST_min_d)
max_attempts = 1000;
bs_pos_list = zeros(numel(BS_list), 2);
for bs_idx = 1:numel(BS_list)
    bs_pos_list(bs_idx, :) = BS_list(bs_idx).Position(1:2);
end

for attempt = 1:max_attempts
    [candidate_pos, side, lane_id] = network_layout.drop_Vehicle_ST(scenario);
    if localPassesMinDistance(candidate_pos, bs_pos_list, BS_ST_min_d) ...
            && localPassesMinDistance(candidate_pos, UE_pos_list(:, 1:2), UE_ST_min_d)
        ST_pos_2D = candidate_pos;
        return;
    end
end

error('Unable to drop UrbanGrid vehicle ST after %d attempts. Check minimum distance settings.', max_attempts);
end

function tf = localPassesMinDistance(pos, reference_pos, min_distance)
if isempty(reference_pos) || isempty(min_distance) || min_distance <= 0
    tf = true;
    return;
end

dist_2d = sqrt(sum((reference_pos(:, 1:2) - pos(1:2)).^2, 2));
tf = all(dist_2d >= min_distance);
end
